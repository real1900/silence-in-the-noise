...
\hypersetup{pdfauthor={Suleman Imdad}, pdftitle={Silence in the Noise: Audio Spectrogram Transformers Are Outlier-Free and INT8-Friendly}}
\graphicspath{{./}}
...
\author{\IEEEauthorblockN{Suleman Imdad}
\IEEEauthorblockA{\textit{Whiting School of Engineering}, \textit{Johns Hopkins University}, Baltimore, MD, USA \\
...
\label{eq:r}
\end{equation}
so the max-to-median ratio $r$ scales bulk quantization error \emph{linearly}. When $r\!\approx\!1$ (AST) the per-element error is $\sim\!0.4\%$; when $r\!\approx\!10$ (ViT-L) it is $\sim\!4\%$, enough to collapse fidelity. We define a model as INT8-faithful if token-state cosine to FP32 exceeds $0.99$, top-$5$ prediction-rank agreement exceeds $0.9$, and the $99^{\text{th}}$-percentile relative $\ell_2$ drift is below $0.25$.
